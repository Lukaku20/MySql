...
select round(avg(sal_emp))as'Salario Promedio' from empleados;
select id_emp, nombre, sex_emp, fec_nac, fec_incorporacion, sal_emp, comision_emp, cargo_emp , nombre_depto from empleados, departamentos where sal_emp>=(select round(avg(sal_emp))as'Salario Promedio' from empleados) and empleados.id_depto=departamentos.id_depto order by nombre_depto;
